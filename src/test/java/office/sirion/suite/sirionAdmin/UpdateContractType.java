...
public class UpdateContractType extends TestSuiteBase {
	...
	public void UpdateContractTypeTest (String contractType, String contractTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +contractTypeUpdate +" set to NO " +count);
		...
		if (!contractTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(contractTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();

			
		if (contractTypeUpdate.length() > 128) {
			...
			Logger.debug("For Contract Type, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Contract Type, " +contractTypeUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_contracttypes_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, contractTypeUpdate, "Contract Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + contractTypeUpdate);

		Logger.debug("Contract Type updated successfully, with Name -- " + contractTypeUpdate);
