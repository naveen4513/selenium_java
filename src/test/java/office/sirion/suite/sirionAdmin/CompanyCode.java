...
public class CompanyCode extends TestSuiteBase {
	...
	public void CompanyCodeCreation (String companyCode) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +companyCode +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Company Code Creation Test -- "+companyCode);
		...
		if (!companyCode.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(companyCode.trim());
			}
		...
		if (companyCode.equalsIgnoreCase("")) {
			...
			Logger.debug("Company Code Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (companyCode.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Company Code, " +companyCode+" --- This name already exists in the system.");
        ...
        Logger.debug("Company Code created successfully, with Name -- " +companyCode);
