...
public class CostCenter extends TestSuiteBase {
	...
	public void CostCenterCreation (String costCenterName, String costCenterDepartment, String costCenterActive) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y")) {
			skip=true;
			throw new SkipException("Runmode for Test Data -- " +costCenterName +" set to NO " +count);
			}
		...
		Logger.debug("Executing Client Admin Cost Center Creation Test -- "+costCenterName);
		...
		getObject("ca_cost_center_plus_button").click();

		getObject("ca_cost_center_name_textbox").clear();
		getObject("ca_cost_center_name_textbox").sendKeys(costCenterName);
		...
		getObject("ca_cost_center_save_button").click();
		...
		if(driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("Cost Center already exists with Name -- " +costCenterName);
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, costCenterName, "Cost Center Org. Preference at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +costCenterName);
