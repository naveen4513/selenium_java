...
public class UpdateRiskType extends TestSuiteBase {
	...
	public void UpdateRiskTypeTest (String riskType, String riskTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +riskTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Risk Type Update Test -- "+riskType);
		...
		if (!riskTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(riskTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (riskTypeUpdate.length() > 128) {
			...
			Logger.debug("For Risk Type, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Risk Type, " +riskTypeUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, riskTypeUpdate, "Risk Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + riskTypeUpdate);

		Logger.debug("Risk Type updated successfully, with Name -- " + riskTypeUpdate);
