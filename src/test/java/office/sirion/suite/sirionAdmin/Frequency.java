...
public class Frequency extends TestSuiteBase {
	...
	public void FrequencyCreation (String frequencyName, String frequencyType, String frequencyRepeats, String frequencyRepeatEvery, String frequencyRepeatOn,
			...
			String frequencyAdvanceCreation, String frequencyBelowMonthly
		...
		Logger.debug("Executing Test Sirion Admin Frequency Creation for -- " + frequencyName);
		...
		if (!frequencyName.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(frequencyName.trim());
			}
		...
		if (frequencyName.equalsIgnoreCase("")) {
			...
			Logger.debug("Frequency Name is Mandatory");
		...
		if (frequencyType.equalsIgnoreCase("")) {
			WebElement e = driver.findElement(By.cssSelector("div._frequencyType_id_idformError.parentFormmasterFrequency.formError"));
			String entityErrors = e.findElement(By.className("formErrorContent")).getText();
			
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("Frequency Type is Mandatory");
		...
		if (frequencyAdvanceCreation.equalsIgnoreCase("")) {
			...
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("Frequency Advance Creation is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
		
		if (frequencyName.length() > 128) {
			WebElement e = driver.findElement(By.cssSelector("div._name_idformError.parentFormmasterFrequency.formError"));
			String entityErrors = e.findElement(By.className("formErrorContent")).getText();
		...
		if (driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Frequency Name, " +frequencyName+" -- This name already exists in the system.");
