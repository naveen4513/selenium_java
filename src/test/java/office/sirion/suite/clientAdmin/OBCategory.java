...
public class OBCategory extends TestSuiteBase {
	...
	public void OBCategoryCreation (String obCategory, String obCategoryOp, String obCategoryActive) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Client Admin OB Category Creation Test -- "+obCategoryOp);
		...
		if (!obCategory.equalsIgnoreCase("")) {
            try {
                new Select(driver.findElement(By.id("_id_id"))).selectByVisibleText(obCategory.trim());
		...
		if (!obCategoryOp.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_alias_id")).sendKeys(obCategoryOp.trim());
			}
		...
		if (obCategory.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._id_idformError.parentFormmasterDNOType.formError"));
			...
			Logger.debug("OB Category is Mandatory");
		...
		if (obCategoryOp.equalsIgnoreCase("")) {
			...
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("OB Category Organization Preference is Mandatory");
		...
		driver.findElement(By.className("dataTables_filter")).findElement(By.tagName("label")).findElement(By.tagName("input")).sendKeys(obCategoryOp);
		Thread.sleep(2000);
        ...
        Logger.debug("OB Category created successfully, with Name -- " + obCategoryOp);
