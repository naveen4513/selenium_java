...
public class ContractService extends TestSuiteBase {
	...
	public void ContractServiceCreation (String contractService) throws InterruptedException {
		count++;
		...
		if (!contractService.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(contractService.trim());
			}
		...
		if (contractService.equalsIgnoreCase("")) {
			...
			Logger.debug("Contract Service Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (contractService.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Contract Service, " +contractService+" --- This name already exists in the system.");
        ...
        Logger.debug("Contract Service created successfully, with Name -- " +contractService);
