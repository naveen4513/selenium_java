...
public class UpdateGovernanceBodyType extends TestSuiteBase {
	...
	public void UpdateGovernanceBodyTypeTest (String governanceBodyType, String governanceBodyTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +governanceBodyTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Governance Body Type Update Test -- "+governanceBodyType);
		...
		if (!governanceBodyTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(governanceBodyTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (governanceBodyTypeUpdate.length() > 128) {
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Governance Body Type, " +governanceBodyTypeUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_GovernanceBodyType_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, governanceBodyTypeUpdate, "Governance Body Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + governanceBodyTypeUpdate);

		Logger.debug("Governance Body Type updated successfully, with Name -- " + governanceBodyTypeUpdate);
