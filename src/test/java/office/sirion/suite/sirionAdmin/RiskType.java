...
public class RiskType extends TestSuiteBase {
	...
	public void RiskTypeCreation (String riskType) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Sirion Admin Risk Type Creation Test -- "+riskType);
		...
		if (!riskType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(riskType.trim());
			}
		...
		if (riskType.equalsIgnoreCase("")) {
			...
			Logger.debug("Risk Type Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (riskType.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Risk Type, " +riskType+" --- This name already exists in the system.");
        ...
        Logger.debug("Risk Type created successfully, with Name -- " +riskType);
