...
public class BusinessLine extends TestSuiteBase {
	...
	public void BusinessLineCreation (String businessLine) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +businessLine +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Business Line Creation Test -- "+businessLine);
		...
		if (!businessLine.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(businessLine.trim());
			}
		...
		if (businessLine.equalsIgnoreCase("")) {
			...
			Logger.debug("Business Line Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (businessLine.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Business Line, " +businessLine+" --- This name already exists in the system.");
        ...
        Logger.debug("Business Line created successfully, with Name -- " +businessLine);
