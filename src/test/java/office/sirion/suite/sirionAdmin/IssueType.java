...
public class IssueType extends TestSuiteBase {
	...
	public void IssueTypeCreation (String issueType) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +issueType +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Issue Type Creation Test -- "+issueType);
		...
		if (!issueType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(issueType.trim());
			}
		...
		if (issueType.equalsIgnoreCase("")) {
			...
			Logger.debug("Issue Type Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (issueType.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Issue Type, " +issueType+" --- This name already exists in the system.");
        ...
        Logger.debug("Issue Type created successfully, with Name -- " +issueType);
