...
public class CRClass extends TestSuiteBase {
	...
	public void CRClassCreation (String crClass) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Sirion Admin CR Class Creation Test -- "+crClass);
		...
		if (!crClass.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(crClass.trim());
			}
		...
		if (crClass.equalsIgnoreCase("")) {
			...
			Logger.debug("CR Class Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (crClass.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For CR Class, " +crClass+" --- This name already exists in the system.");
        ...
        Logger.debug("CR Class created successfully, with Name -- " +crClass);
