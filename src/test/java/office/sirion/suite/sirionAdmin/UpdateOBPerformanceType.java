...
public class UpdateOBPerformanceType extends TestSuiteBase {
	...
	public void UpdateOBPerformanceTypeTest (String obPerformanceType, String obPerformanceTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +obPerformanceTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin OB Performance Type Creation Test -- "+obPerformanceType);
		...
		driver.findElement(By.className("dataTables_filter")).findElement(By.tagName("label")).findElement(By.tagName("input")).sendKeys(obPerformanceType);
		Thread.sleep(2000);
        ...
        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterPerformanceType_name_name_id")).getText();
		...
		if (!obPerformanceTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(obPerformanceTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (obPerformanceTypeUpdate.length() > 128) {
			...
			Logger.debug("For OB Performance Type, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For OB Performance Type, " +obPerformanceTypeUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterPerformanceType_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, obPerformanceTypeUpdate, "OB Performance Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + obPerformanceTypeUpdate);

		Logger.debug("OB Performance Type updated successfully, with Name -- " + obPerformanceTypeUpdate);
