...
public class UpdateContractPaper extends TestSuiteBase {
	...
	public void UpdateContractPaperTest (String contractPaper, String contractPaperUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y")) {
			throw new SkipException("Runmode for Test Data -- " +contractPaperUpdate +" set to NO " +count);
			}
		...
		Logger.debug("Executing Sirion Admin Contract Paper Creation Test -- "+contractPaper);
		...
		driver.findElement(By.className("dataTables_filter")).findElement(By.tagName("label")).findElement(By.tagName("input")).sendKeys(contractPaper);
		Thread.sleep(2000);
        
        driver.findElement(By.xpath(".//*[@id='l_com_sirionlabs_model_ContractPaper']/tbody/tr/td[1]/a/div[@sort='"+ contractPaper +"']")).click();

        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_ContractPaper_name_name_id")).getText();
		...
		if (!contractPaperUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(contractPaperUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (contractPaperUpdate.length() > 256) {
			...
			Logger.debug("For Contract Paper, --- Maximum 256 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Contract Paper, " +contractPaperUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_ContractPaper_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, contractPaperUpdate, "Contract Paper Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + contractPaperUpdate);

		Logger.debug("Contract Paper updated successfully, with Name -- " + contractPaperUpdate);
