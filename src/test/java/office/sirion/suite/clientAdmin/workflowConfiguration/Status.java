...
public class Status extends TestSuiteBase {
	...
	public void StatusCreation (String status, String statusDisplay, String statusEntity, String statusType, String statusColor
			) throws InterruptedException {

		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test data -- " + status +" set to NO " +count);

		openBrowser();
		clientAdminLogin(CONFIG.getProperty("clientAdminURL"), CONFIG.getProperty("clientAdminUsername"), CONFIG.getProperty("clientAdminPassword"));

		Logger.debug("Executing Test Client Admin Status Creation for -- " + status);

		driver.get(CONFIG.getProperty("clientAdminURL"));
		//
		fluentWaitMethod(driver.findElement(By.linkText("Status")));
		...
		if (!status.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(status.trim());
			}

		if (!statusDisplay.equalsIgnoreCase("")) {
			driver.findElement(By.id("_description_id")).clear();
			driver.findElement(By.id("_description_id")).sendKeys(statusDisplay.trim());
			}

		if (!statusEntity.equalsIgnoreCase(""))
			new Select(driver.findElement(By.id("_entityType.id_id"))).selectByVisibleText(statusEntity.trim());

		if (!statusType.equalsIgnoreCase(""))
			new Select(driver.findElement(By.id("_workFlowStatusType.id_id"))).selectByVisibleText(statusType.trim());

		if (!statusColor.equalsIgnoreCase(""))
			new Select(driver.findElement(By.id("_colorId_id"))).selectByVisibleText(statusColor.trim());
		...
		if(driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			if (entityErrors.contains("This name already exists in the system."))
				Logger.debug("For Status, " +status+" -- This name already exists in the system.");
		...
		Assert.assertEquals(entityAlert, "Status created successfully");

		driver.findElement(By.cssSelector("button.ui-button.ui-widget.ui-state-default.ui-corner-all.ui-button-text-only")).click();

		Logger.debug("Test Client Admin Status Creation for -- " + status + " is PASSED");
		driver.get(CONFIG.getProperty("clientAdminURL"));
		}
