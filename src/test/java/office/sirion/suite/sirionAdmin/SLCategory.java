...
public class SLCategory extends TestSuiteBase {
	...
	public void SLCategoryCreation (String slCategory) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +slCategory +" set to NO " +count);
		...
		Logger.debug("Executing Test Sirion Admin SL Category Creation Test for -- " + slCategory);
		...
		if (!slCategory.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(slCategory.trim());
			}
		...
		if (slCategory.equalsIgnoreCase("")) {
			...
			Logger.debug("SL Category Name is Mandatory");

			driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }

		if (slCategory.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For SL Category, " +slCategory+" -- This name already exists in the system.");
		...
		Logger.debug("Test Sirion Admin SL Category Creation Test for -- " + slCategory + " is PASSED");
		driver.get(CONFIG.getProperty("sirionAdminURL"));
		}
