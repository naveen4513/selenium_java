...
public class ContractAuditLog extends TestSuiteBase {
	String result = null;
	...
	public void ContractAuditLogTest (String coName, String coTitle, String coAgreementNo, String coContractNo, String coBrief, String coBusinessUnit,
			...
			String coParent) throws Exception {
	    ...
	    Logger.debug("Executing Test Case Contract Audit Log with Name -- " + coName);
		...
		locateElementBy("csl_show_page_bottom_tab").findElement(By.cssSelector("div.submit.freeze"));
			} catch (NoSuchElementException e){locateElementBy("csl_show_page_bottom_tab").findElement(By.cssSelector("div.submit.freeze"));}
		// CONTRACT - SHOW PAGE - EDIT BUTTON
	    WebElement elementEdit = locateElementBy("csl_show_page_bottom_tab").findElement(By.cssSelector("div.submit.freeze"));
		...
		if (locateElementBy("co_create_page_brief_textarea").findElements(By.tagName("textarea")).size()!=0)
			...
			locateElementBy("co_update_page_brief_textarea").clear();
            locateElementBy("co_update_page_brief_textarea").sendKeys(coBrief.trim());
		...
		try {
			if (locateElementBy("csl_pop_up_id").getText().contains("Current Date is different for the selected Time Zone"))
	    ...
	    driver.findElement(By.className("tabs-outer-wrap")).findElement(By.cssSelector("div.tab-scroller.tab-scroller-right.right-arrow-tabs.positionRightOfFirst")).click();

	    locateElementBy("co_show_page_creditearnback_audit_log_tab").click();

	    Logger.debug("Test Case Contract Audit Log for -- " + coName + " -- is STARTED");

	    WebElement elementOddRowSelected;

	    if (!coDocumentName.equalsIgnoreCase(""))
			elementOddRowSelected = driver.findElement(By.id("table_49")).findElements(By.className("even")).get(0);
		else
			elementOddRowSelected = driver.findElement(By.id("table_49")).findElements(By.className("odd")).get(0);

	    Assert.assertEquals(elementOddRowSelected.findElement(By.cssSelector("td.col_wrap_action_name")).getText(), "Updated");

	    if (!coRequestedBy.equalsIgnoreCase(""))
			Assert.assertEquals(elementOddRowSelected.findElement(By.cssSelector("td.col_wrap_requested_by")).getText(), coRequestedBy);
		else
	    ...
	    if (!coComments.equalsIgnoreCase(""))
	    ...
	    Logger.debug("Test Case Contract Audit Log for -- " + coName + " -- is PASSED");
        driver.get(CONFIG.getProperty("endUserURL"));
	    }
