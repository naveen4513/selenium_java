...
public class UpdateCompanyPosition extends TestSuiteBase {
	...
	public void UpdateCompanyPositionTest (String companyPosition, String companyPositionUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +companyPositionUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Company Position Update Test -- "+companyPosition);
		...
		if (!companyPositionUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(companyPositionUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (companyPositionUpdate.length() > 128) {
			...
			Logger.debug("For Company Position, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Company Position, " +companyPositionUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, companyPositionUpdate, "Company Position Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + companyPositionUpdate);

		Logger.debug("Company Position updated successfully, with Name -- " + companyPositionUpdate);
