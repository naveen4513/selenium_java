...
public class UpdateAgreementType extends TestSuiteBase {
	...
	public void UpdateAgreementTypeTest (String agreementType, String agreementTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +agreementTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Agreement Type Update Test -- "+agreementType);
        ...
        Assert.assertEquals(entityTypeShowPage, agreementType, "Agreement Type Name at show page is -- " +entityTypeShowPage+ " instead of -- " +agreementType);
		...
		if (!agreementTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(agreementTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (agreementTypeUpdate.length() > 128) {
			...
			Logger.debug("For Agreement Type, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Agreement Type, " +agreementTypeUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, agreementTypeUpdate, "Agreement Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + agreementTypeUpdate);

		Logger.debug("Agreement Type updated successfully, with Name -- " + agreementTypeUpdate);
