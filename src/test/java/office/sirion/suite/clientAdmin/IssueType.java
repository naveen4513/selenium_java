...
public class IssueType extends TestSuiteBase {
	...
	public void IssueTypeCreation (String issueType, String issueTypeOp, String issueTypeActive) throws InterruptedException {
		...
		Logger.debug("Executing Client Admin Issue Type Creation Test -- "+issueTypeOp);
		...
		if (!issueType.equalsIgnoreCase("")) {
		    try {
                new Select(driver.findElement(By.id("_systemEntity.id_id"))).selectByVisibleText(issueType.trim());
		...
		if (issueType.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._systemEntity_id_idformError.parentFormissueType.formError"));
			...
			Logger.debug("Issue Type is Mandatory");
		...
		if (issueTypeOp.equalsIgnoreCase("")) {
			...
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("Issue Type Organization Preference is Mandatory");
        ...
        Logger.debug("Issue Type created successfully, with Name -- " + issueTypeOp);
