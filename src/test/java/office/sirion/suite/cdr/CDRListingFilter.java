...
import office.sirion.suite.commonFilters.commonFilter;
...
public class CDRListingFilter extends TestSuiteBase {
	String result = null;
	...
	public void CDRListingFilterTest (
			) throws InterruptedException {
		
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- CONTRACT DRAFT REQUEST -- is set to NO " +count);

		openBrowser();
	    endUserLogin(CONFIG.getProperty("endUserURL"), CONFIG.getProperty("endUserUsername"), CONFIG.getProperty("endUserPassword"));
	    
	    Logger.debug("Executing Filter Test Case for -- CONTRACT DRAFT REQUEST");
	    ...
	    if (filtersCount==0) {
	    	org.testng.Assert.fail("For CONTRACT DRAFT REQUEST -- Listing there are No Corresponding Filters Available");
	    ...
	    for (int filterIndex=0; filterIndex<filtersCount; filterIndex++) {
	    	...
	    	if (resultCount>0) {
	    		Logger.error("For CONTRACT DRAFT REQUEST -- Filter -- " +filterName+ " -- is not working");
	    		filterTestStatus = filterTestStatus+resultCount;
	    		resultCount=0;
	    		break;
	    		}
	    	else
		    	Logger.debug("For CONTRACT DRAFT REQUEST -- Filter Status for -- " + filterName + " -- TRUE");
	    ...
	    if (filterTestStatus>0)
	    	org.testng.Assert.fail("For CONTRACT DRAFT REQUEST --- Filter is not working");
	    else
	    	Logger.debug("For CONTRACT DRAFT REQUEST -- Filter is Working as Expected");
