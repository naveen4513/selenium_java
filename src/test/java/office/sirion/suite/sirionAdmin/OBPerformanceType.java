...
public class OBPerformanceType extends TestSuiteBase {
	...
	public void OBPerformanceTypeCreation (String obPerformanceType) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +obPerformanceType +" set to NO " +count);
		...
		if (!obPerformanceType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(obPerformanceType.trim());
			}
		...
		if (obPerformanceType.equalsIgnoreCase("")) {
			...
			Logger.debug("OB Performance Type Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (obPerformanceType.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For OB Performance Type, " +obPerformanceType+" --- This name already exists in the system.");
        ...
        Logger.debug("OB Performance Type created successfully, with Name -- " +obPerformanceType);
