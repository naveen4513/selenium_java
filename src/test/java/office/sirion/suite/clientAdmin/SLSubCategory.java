...
public class SLSubCategory extends TestSuiteBase {
	...
	public void SLSubCategoryCreation (String slCategory, String slSubCategory, String slSubCategoryOp, String slSubCategoryActive
		...
		Logger.debug("Executing Client Admin SL Sub-Category Creation Test -- "+slSubCategoryOp);
		...
		if (!slSubCategory.equalsIgnoreCase("")) {
            try {
                new Select(driver.findElement(By.id("_id_id"))).selectByVisibleText(slSubCategory.trim());
		...
		if (!slSubCategoryOp.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(slSubCategoryOp.trim());
			}
		...
		if (slSubCategory.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._id_idformError.parentFormmasterSlasubCategory.formError"));
			...
			Logger.debug("SL Sub-Category is Mandatory");
		...
		if (slSubCategoryOp.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._name_idformError.parentFormmasterSlasubCategory.formError"));
			String entityErrors = e.findElement(By.className("formErrorContent")).getText();
			
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("SL Sub-Category Organization Preference is Mandatory");
		...
		if (slSubCategoryOp.length() > 128) {
			...
			Logger.debug("For SL Sub-Category Organization Preference, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			if (entityErrors.contains("This name already exists in the system."))
				Logger.debug("For SL Sub-Category, " +slSubCategoryOp+" --- This name already exists in the system.");
        ...
        Thread.sleep(2000);
/*
		driver.findElement(By.className("dataTables_filter")).findElement(By.tagName("label")).findElement(By.tagName("input")).sendKeys(slSubCategoryOp);
		Thread.sleep(2000);
*/
        ...
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, slSubCategoryOp, "SL Sub-Category Name at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +slSubCategoryOp);

        Logger.debug("SL Sub-Category created successfully, with Name -- " + slSubCategoryOp);
