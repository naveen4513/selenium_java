...
public class UpdateSLSubCategory extends TestSuiteBase {
	...
	public void UpdateSLSubCategoryTest (String slSubCategory, String slSubCategoryUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +slSubCategoryUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin SL Sub-Category Creation Test -- "+slSubCategory);
		
		driver.get(CONFIG.getProperty("sirionAdminURL"));
		fluentWaitMethod(driver.findElement(By.linkText("SL Sub-Category")));

		driver.findElement(By.linkText("SL Sub-Category")).click();
        ...
        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterSlasubCategory_name_name_id")).getText();
		...
		if (!slSubCategoryUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(slSubCategoryUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (slSubCategoryUpdate.length() > 128) {
			...
			Logger.debug("For SL Sub-Category, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For SL Sub-Category, " +slSubCategoryUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterSlasubCategory_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, slSubCategoryUpdate, "SL Sub-Category Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + slSubCategoryUpdate);

		Logger.debug("SL Sub-Category updated successfully, with Name -- " + slSubCategoryUpdate);
