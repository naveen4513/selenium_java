...
public class UpdateOBCategory extends TestSuiteBase {
	...
	public void UpdateOBCategoryTest (String obCategory, String obCategoryUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +obCategoryUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin OB Category Creation Test -- "+obCategory);
        ...
        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterDNOType_name_name_id")).getText();
        Assert.assertEquals(entityTypeShowPage, obCategory, "OB Category Name at show page is -- " +entityTypeShowPage+ " instead of -- " +obCategory);
		...
		if (!obCategoryUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(obCategoryUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (obCategoryUpdate.length() > 128) {
			...
			Logger.debug("For OB Category, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For OB Category, " +obCategoryUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterDNOType_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, obCategoryUpdate, "OB Category Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + obCategoryUpdate);

		Logger.debug("OB Category updated successfully, with Name -- " + obCategoryUpdate);
