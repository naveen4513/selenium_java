...
public class UpdateTransactionType extends TestSuiteBase {
	...
	public void UpdateTransactionTypeTest (String transactionType, String transactionTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +transactionTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Transaction Type Update Test -- "+transactionType);
		...
		if (!transactionTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(transactionTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (transactionTypeUpdate.length() > 128) {
			...
			Logger.debug("For Transaction Type, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Transaction Type, " +transactionTypeUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, transactionTypeUpdate, "Transaction Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + transactionTypeUpdate);

		Logger.debug("Transaction Type updated successfully, with Name -- " + transactionTypeUpdate);
