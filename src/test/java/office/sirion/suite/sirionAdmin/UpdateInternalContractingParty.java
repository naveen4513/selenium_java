...
public class UpdateInternalContractingParty extends TestSuiteBase {
	...
	public void UpdateInternalContractingPartyTest (String internalContractingParty, String internalContractingPartyUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +internalContractingPartyUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Internal Contracting Party Update Test -- "+internalContractingParty);
		
		driver.get(CONFIG.getProperty("sirionAdminURL"));
		fluentWaitMethod(driver.findElement(By.linkText("Internal Contracting Party")));

		driver.findElement(By.linkText("Internal Contracting Party")).click();
		...
		new Select(driver.findElement(By.xpath("//select[@name='l_com_sirionlabs_model_icp_length']"))).selectByIndex(3);
        ...
        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_icp_name_name_id")).getText();
        Assert.assertEquals(entityTypeShowPage, internalContractingParty, "Internal Contracting Party Name at show page is -- " +entityTypeShowPage+ " instead of -- " +internalContractingParty);
		...
		if (!internalContractingPartyUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(internalContractingPartyUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (internalContractingPartyUpdate.length() > 128) {
			...
			Logger.debug("For Internal Contracting Party, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Internal Contracting Party, " +internalContractingPartyUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_icp_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, internalContractingPartyUpdate, "Internal Contracting Party Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + internalContractingPartyUpdate);

		Logger.debug("Internal Contracting Party updated successfully, with Name -- " + internalContractingPartyUpdate);
