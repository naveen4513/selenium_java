...
public class UpdateContractService extends TestSuiteBase {
	...
	public void UpdateContractServiceTest (String contractService, String contractServiceUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +contractServiceUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Contract Service Creation Test -- "+contractService);
		...
		driver.findElement(By.className("dataTables_filter")).findElement(By.tagName("label")).findElement(By.tagName("input")).sendKeys(contractService);
		Thread.sleep(2000);
        ...
        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterContractService_name_name_id")).getText();
		...
		if (!contractServiceUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(contractServiceUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (contractServiceUpdate.length() > 128) {
			...
			Logger.debug("For Contract Service, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Contract Service, " +contractServiceUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterContractService_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, contractServiceUpdate, "Contract Service Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + contractServiceUpdate);

		Logger.debug("Contract Service updated successfully, with Name -- " + contractServiceUpdate);
