...
public class DiscrepancyReasonType extends TestSuiteBase {
	...
	public void DiscrepancyReasonTypeCreation (String discrepancyReasonType) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Sirion Admin Discrepancy Reason Type Creation Test -- "+discrepancyReasonType);
		...
		if (!discrepancyReasonType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(discrepancyReasonType.trim());
			}
		...
		if (discrepancyReasonType.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Discrepancy Reason Type, " +discrepancyReasonType+" --- This name already exists in the system.");
        ...
        Logger.debug("Discrepancy Reason Type created successfully, with Name -- " +discrepancyReasonType);
