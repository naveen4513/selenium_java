...
public class Service extends TestSuiteBase {
	...
	public void ServiceCreation (String service) throws InterruptedException {
		count++;
		...
		if (!service.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(service.trim());
			}
		...
		if (service.equalsIgnoreCase("")) {
			...
			Logger.debug("Service Name is Mandatory");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Service, " +service+" --- This name already exists in the system.");
        ...
        Logger.debug("Service created successfully, with Name -- " +service);
