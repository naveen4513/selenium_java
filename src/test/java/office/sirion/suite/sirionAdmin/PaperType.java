...
public class PaperType extends TestSuiteBase {
	...
	public void PaperTypeCreation (String paperType) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Sirion Admin Paper Type Creation Test -- "+paperType);
		...
		if (!paperType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(paperType.trim());
			}
		...
		if (paperType.equalsIgnoreCase("")) {
			...
			Logger.debug("Paper Type Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (paperType.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Paper Type, " +paperType+" --- This name already exists in the system.");
        ...
        Logger.debug("Paper Type created successfully, with Name -- " +paperType);
