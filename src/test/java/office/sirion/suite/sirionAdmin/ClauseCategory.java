...
public class ClauseCategory extends TestSuiteBase {
	...
	public void ClauseCategoryCreation (String clauseCategory) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Sirion Admin Clause Category Creation Test -- "+clauseCategory);
		...
		if (!clauseCategory.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(clauseCategory.trim());
			}
		...
		if (clauseCategory.equalsIgnoreCase("")) {
			...
			Logger.debug("Clause Category Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (clauseCategory.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Clause Category, " +clauseCategory+" --- This name already exists in the system.");
        ...
        Logger.debug("Clause Category created successfully, with Name -- " +clauseCategory);
