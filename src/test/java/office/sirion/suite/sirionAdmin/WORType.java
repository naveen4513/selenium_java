...
public class WORType extends TestSuiteBase {
	...
	public void WORTypeCreation (String worType) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Sirion Admin WOR Type Creation Test -- "+worType);
		...
		if (!worType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(worType.trim());
			}
		...
		if (worType.equalsIgnoreCase("")) {
			...
			Logger.debug("WOR Type Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (worType.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For WOR Type, " +worType+" --- This name already exists in the system.");
        ...
        Logger.debug("WOR Type created successfully, with Name -- " +worType);
