...
public class VendorHierarchyUpdate extends TestSuiteBase {
	String result = null;
	...
	public void VendorHierarchyUpdateTest (String Name, String Alias, String Address, String ECCID, String Functions, String Services,
		...
		if (!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " + Name + " set to NO -- " + count);
	    ...
	    Logger.debug("Executing Test Case Vendor Hierarchy Update for -- " + Name);
		...
		fluentWaitMethod(driver.findElement(By.name("cr_length")));
		...
		new Select(locateElementBy("entity_listing_page_display_dropdown_link")).selectByVisibleText(CONFIG.getProperty("maxEntityListingDropdown"));
			...
			String table_info = driver.findElement(By.xpath("//div[@class='dataTables_info']")).getText();
			String total_count = table_info.split("of ")[1].split(" entries")[0];
			System.out.print(total_count);
			for (int i = 0; i <(Integer.parseInt(total_count)/100) ; i++) {
                      try{
						  driver.findElement(By.xpath("//td[text()='"+ParentEntity+"']//parent::tr//a"));
						  break;
					  }catch(Exception e){
                      	Logger.debug("Element not found on page "+(i+1));
                      	driver.findElement(By.xpath("//a[@class='paginate_button ' and text()="+(i+2)+"]")).click();
					  }
			}
		...
		if (!ParentEntity.equalsIgnoreCase("") && ParentEntity.length()<=30)
			driver.findElement(By.xpath("//td[text()='"+ParentEntity+"']//parent::tr//a")).click();
	    ...
	    Logger.debug("Test Case Vendor Hierarchy Update for -- " + Name + " is PASSED");
        driver.get(CONFIG.getProperty("endUserURL"));
	    }
