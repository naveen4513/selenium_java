...
public class WORCommunication extends TestSuiteBase {
  String result = null;
  ...
  public void WORCommunicationTest(String worComments, String worActualDate, String worRequestedBy, String worChangeRequest,
      String worUploadFile) throws InterruptedException {
    ...
    fluentWaitMethod(locateElementBy("entity_listing_page_first_entry_link_wor"));
    ...
    addFieldValue("wor_comment_textarea", worComments);
    locateElementBy("wor_comment_attachment").click();
	addFieldValue("wor_requested_by_drop_down", worRequestedBy);
	...
	if (!worUploadFile.equalsIgnoreCase("")) {
		locateElementBy("entity_create_page_upload_browse_button").sendKeys(System.getProperty("user.dir")+"\\file-upload\\Child GB\\" + worUploadFile);
    ...
    if (getObject("wor_error_notifications") != null) {
    ...
    waitF.until(ExpectedConditions.visibilityOf(driver.findElement(By.id("table_931_wrapper"))));
	 
	 Assert.assertEquals(locateElementBy("wor_communication_status").getText(), "Comment/Attachment");
      Logger.debug("Test Case Contract Communication for -- " + locateElementBy("wor_show_page_name").getText() + " -- is STARTED");

      verifyCommunicationTab(worComments, worUploadFile);

      Logger.debug("Test Case Contract Communication for -- " + locateElementBy("wor_show_page_name").getText() + " -- is Finished");
