...
public class UpdateIndustryType extends TestSuiteBase {
	...
	public void UpdateIndustryTypeTest (String industryType, String industryTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +industryTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Industry Type Update Test -- "+industryType);
		...
		if (!industryTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(industryTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (industryTypeUpdate.length() > 128) {
			...
			Logger.debug("For Industry Type, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Industry Type, " +industryTypeUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, industryTypeUpdate, "Industry Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + industryTypeUpdate);

		Logger.debug("Industry Type updated successfully, with Name -- " + industryTypeUpdate);
