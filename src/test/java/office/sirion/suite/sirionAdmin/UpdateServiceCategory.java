...
public class UpdateServiceCategory extends TestSuiteBase {
	...
	public void UpdateServiceCategoryTest (String serviceCategory, String serviceCategoryUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +serviceCategoryUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Service Category Creation Test -- "+serviceCategory);
        ...
        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterServiceCategory_name_name_id")).getText();
		...
		if (!serviceCategoryUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(serviceCategoryUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (serviceCategoryUpdate.length() > 128) {
			...
			Logger.debug("For Service Category, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Service Category, " +serviceCategoryUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterServiceCategory_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, serviceCategoryUpdate, "Service Category Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + serviceCategoryUpdate);

		Logger.debug("Service Category updated successfully, with Name -- " + serviceCategoryUpdate);
