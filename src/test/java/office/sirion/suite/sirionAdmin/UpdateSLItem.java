...
public class UpdateSLItem extends TestSuiteBase {
	...
	public void UpdateSLItemTest (String slItem, String slItemUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +slItemUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin SL Item Creation Test -- "+slItem);
		...
		if (!slItemUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(slItemUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (slItemUpdate.length() > 128) {
			...
			Logger.debug("For SL Item, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For SL Item, " +slItemUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterSlaitem_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, slItemUpdate, "SL Item Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + slItemUpdate);

		Logger.debug("SL Item updated successfully, with Name -- " + slItemUpdate);
