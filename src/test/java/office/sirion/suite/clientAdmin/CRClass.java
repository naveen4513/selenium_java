...
public class CRClass extends TestSuiteBase {
	...
	public void CRClassCreation (String crClass, String crClassOp, String crClassActive) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Client Admin CR Class Creation Test -- "+crClassOp);
		...
		if (!crClass.equalsIgnoreCase("")) {
            try {
                new Select(driver.findElement(By.id("_systemEntity.id_id"))).selectByVisibleText(crClass.trim());
		...
		if (crClass.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._systemEntity_id_idformError.parentFormcrClass.formError"));
			...
			Logger.debug("CR Class is Mandatory");
		...
		if (crClassOp.equalsIgnoreCase("")) {
			...
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("CR Class Organization Preference is Mandatory");
        ...
        Logger.debug("CR Class created successfully, with Name -- " + crClassOp);
