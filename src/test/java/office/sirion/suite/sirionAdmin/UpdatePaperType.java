...
public class UpdatePaperType extends TestSuiteBase {
	...
	public void UpdatePaperTypeTest (String paperType, String paperTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +paperTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Paper Type Update Test -- "+paperType);
		...
		if (!paperTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(paperTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (paperTypeUpdate.length() > 128) {
			...
			Logger.debug("For Paper Type, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Paper Type, " +paperTypeUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, paperTypeUpdate, "Paper Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + paperTypeUpdate);

		Logger.debug("Paper Type updated successfully, with Name -- " + paperTypeUpdate);
