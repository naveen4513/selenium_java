...
public class CRCreation extends TestSuiteBase {
  ...
  public void CRCreationTest (String crTitle, String crDescription, String crContractReferences, String crTimezone, String crClass, String crType,
		  ...
		  String crSupplier, String crSourceType, String crSourceName
	    ...
	    Logger.debug("Executing Test Case CR Creation with Title ---- " + crTitle + " under Source ---- " + crSourceType);
		
	    driver.get(CONFIG.getProperty("endUserURL"));
		fluentWaitMethod(locateElementBy("cr_quick_link"));

	    locateElementBy("cr_quick_link").click();
		fluentWaitMethod(locateElementBy("cr_entity_listing_page_plus_button"));
	    
	    locateElementBy("cr_entity_listing_page_plus_button").click();

		addFieldValue("entity_global_create_page_supplier_dropdown", crSupplier.trim());
		addFieldValue("entity_global_create_page_source_type_dropdown", crSourceType.trim());  
		
		if(!crSourceType.trim().equalsIgnoreCase("Supplier")){
		addFieldValue("entity_global_create_page_source_name_title_dropdown", crSourceName.trim());
		}
	    ...
	    // Change Request - Create Page - BASIC INFORMATION
		...
		// Change Request - Create Page - CONTRACTING ENTITY
		...
		// Change Request - Create Page - FUNCTIONS
		addFieldValue("cr_create_page_services_multi_dropdown", crFunctions);
		...
		// Change Request - Create Page - GEOGRAPHY
		...
		// Change Request - Create Page - FINANCIAL INFORMATION
		...
		// Change Request - Create Page - PROJECT INFORMATION
		...
		// CHANGE REQUESTS - CREATE PAGE - FIELD VALIDATIONS
		...
		if (driver.findElements(By.className("success-icon")).size() != 0) {
			...
			Logger.debug("Change Request created successfully with Change Request ID " + entityID);
			...
			Logger.debug("Change Request ID on show page has been verified");
			}
