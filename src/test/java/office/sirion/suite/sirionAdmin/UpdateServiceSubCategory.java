...
public class UpdateServiceSubCategory extends TestSuiteBase {
	...
	public void UpdateServiceSubCategoryTest (String serviceSubCategory, String serviceSubCategoryUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +serviceSubCategoryUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Service Sub-Category Creation Test -- "+serviceSubCategory);
		
		driver.get(CONFIG.getProperty("sirionAdminURL"));
		fluentWaitMethod(driver.findElement(By.linkText("Service Sub-Category")));
		...
		driver.findElement(By.className("dataTables_filter")).findElement(By.tagName("label")).findElement(By.tagName("input")).sendKeys(serviceSubCategory);
		Thread.sleep(2000);
        ...
        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterServiceSubCategory_name_name_id")).getText();
		...
		if (!serviceSubCategoryUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(serviceSubCategoryUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();

			
		if (serviceSubCategoryUpdate.length() > 128) {
			...
			Logger.debug("For Service Sub-Category, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Service Sub-Category, " +serviceSubCategoryUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterServiceSubCategory_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, serviceSubCategoryUpdate, "Service Sub-Category Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + serviceSubCategoryUpdate);

		Logger.debug("Service Sub-Category updated successfully, with Name -- " + serviceSubCategoryUpdate);
