...
public class UpdateClauseType extends TestSuiteBase {
	...
	public void UpdateClauseTypeTest (String clauseType, String clauseTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +clauseTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Clause Type Update Test -- "+clauseType);
		...
		if (!clauseTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(clauseTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (clauseTypeUpdate.length() > 128) {
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Clause Type, " +clauseTypeUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, clauseTypeUpdate, "Clause Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + clauseTypeUpdate);

		Logger.debug("Clause Type updated successfully, with Name -- " + clauseTypeUpdate);
