...
public class UpdateFrequency extends TestSuiteBase {
	...
	public void UpdateFrequencyTest (String frequency, String frequencyOp, String frequencyActive) throws InterruptedException {
		count++;
		...
		driver.findElement(By.className("dataTables_filter")).findElement(By.tagName("label")).findElement(By.tagName("input")).sendKeys(frequency);
		Thread.sleep(2000);
		...
		if (!frequencyOp.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(frequencyOp.trim());
			}
		...
		} else if (frequencyActive.equalsIgnoreCase("No")) {
			if (driver.findElement(By.id("_active_id")).isSelected())
				driver.findElement(By.id("_active_id")).click();
		}
        ...
        Logger.debug("Frequency updated successfully, with Name -- " + frequencyOp);
