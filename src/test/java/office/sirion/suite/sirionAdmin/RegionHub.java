...
public class RegionHub extends TestSuiteBase {
	...
	public void RegionHubCreation (String regionHub) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +regionHub +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Region/Hub Creation Test -- "+regionHub);
		...
		if (!regionHub.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(regionHub.trim());
			}
		...
		if (regionHub.equalsIgnoreCase("")) {
			...
			Logger.debug("Region/Hub Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (regionHub.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Region/Hub, " +regionHub+" --- This name already exists in the system.");
        ...
        Logger.debug("Region/Hub created successfully, with Name -- " +regionHub);
