...
public class UpdateSLCategory extends TestSuiteBase {
	...
	public void UpdateSLCategoryTest (String slCategory, String slCategoryUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +slCategoryUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin SL Category Creation Test -- "+slCategory);
        ...
        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterSlacategory_name_name_id")).getText();
		...
		if (!slCategoryUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(slCategoryUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (slCategoryUpdate.length() > 128) {
			...
			Logger.debug("For SL Category, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For SL Category, " +slCategoryUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterSlacategory_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, slCategoryUpdate, "SL Category Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + slCategoryUpdate);

		Logger.debug("SL Category updated successfully, with Name -- " + slCategoryUpdate);
