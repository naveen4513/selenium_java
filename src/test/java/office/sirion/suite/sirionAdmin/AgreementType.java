...
public class AgreementType extends TestSuiteBase {
	...
	public void AgreementTypeCreation (String agreementType) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Sirion Admin Agreement Type Creation Test -- "+agreementType);
		...
		if (!agreementType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(agreementType.trim());
			}
		...
		if (agreementType.equalsIgnoreCase("")) {
			...
			Logger.debug("Agreement Type Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (agreementType.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Agreement Type, " +agreementType+" --- This name already exists in the system.");
        ...
        Logger.debug("Agreement Type created successfully, with Name -- " +agreementType);
