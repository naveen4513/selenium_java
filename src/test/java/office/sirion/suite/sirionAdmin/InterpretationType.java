...
public class InterpretationType extends TestSuiteBase {
	...
	public void InterpretationTypeCreation (String interpretationType) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +interpretationType +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Interpretation Type Creation Test -- "+interpretationType);
		...
		if (!interpretationType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(interpretationType.trim());
			}
		...
		if (interpretationType.equalsIgnoreCase("")) {
			...
			Logger.debug("Interpretation Type Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (interpretationType.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Interpretation Type, " +interpretationType+" --- This name already exists in the system.");
        ...
        Logger.debug("Interpretation Type created successfully, with Name -- " +interpretationType);
