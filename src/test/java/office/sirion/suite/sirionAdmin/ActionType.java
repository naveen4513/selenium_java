...
public class ActionType extends TestSuiteBase {
	...
	public void ActionTypeCreation (String actionType) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Test Sirion Super Admin Action Type Creation for -- "+actionType);
		...
		if (!actionType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(actionType.trim());
			}
		...
		if (actionType.equalsIgnoreCase("")) {
			...
			Logger.debug("Action Type Name is Mandatory");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Action Type, " +actionType+" -- This name already exists in the system.");
		...
		Logger.debug("Test Sirion Super Admin Action Type Creation for -- " + actionType + " is PASSED");
		driver.get(CONFIG.getProperty("sirionAdminURL"));
		}
