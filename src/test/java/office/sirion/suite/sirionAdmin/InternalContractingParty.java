...
public class InternalContractingParty extends TestSuiteBase {
	...
	public void InternalContractingPartyCreation (String internalContractingParty) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +internalContractingParty +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Internal Contracting Party Creation Test -- "+internalContractingParty);
		...
		if (!internalContractingParty.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(internalContractingParty.trim());
			}
		...
		if (internalContractingParty.equalsIgnoreCase("")) {
			...
			Logger.debug("Internal Contracting Party Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (internalContractingParty.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Internal Contracting Party, " +internalContractingParty+" --- This name already exists in the system.");
        ...
        Logger.debug("Internal Contracting Party created successfully, with Name -- " +internalContractingParty);
