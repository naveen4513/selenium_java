...
public class Market extends TestSuiteBase {
	...
	public void MarketCreation (String market) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +market +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Market Creation Test -- "+market);
		...
		if (!market.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(market.trim());
			}
		...
		if (market.equalsIgnoreCase("")) {
			...
			Logger.debug("Market Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (market.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Market, " +market+" --- This name already exists in the system.");
        ...
        Logger.debug("Market created successfully, with Name -- " +market);
