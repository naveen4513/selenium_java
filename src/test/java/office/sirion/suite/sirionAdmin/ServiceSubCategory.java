...
public class ServiceSubCategory extends TestSuiteBase {
	...
	public void ServiceSubCategoryCreation (String serviceSubCategory) throws InterruptedException {
		count++;
		...
		if (!serviceSubCategory.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(serviceSubCategory.trim());
			}
		...
		if (serviceSubCategory.equalsIgnoreCase("")) {
			...
			Logger.debug("Service Sub-Category Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (serviceSubCategory.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Service Sub-Category, " +serviceSubCategory+" --- This name already exists in the system.");
        ...
        Logger.debug("Service Sub-Category created successfully, with Name -- " +serviceSubCategory);
