...
public class Service extends TestSuiteBase {
	...
	public void ServiceCreation (String function, String service, String serviceOp, String serviceActive) throws InterruptedException {
		...
		Logger.debug("Executing Client Admin Service Creation Test -- "+serviceOp);
		...
		if (!service.equalsIgnoreCase("")) {
            try {
                new Select(driver.findElement(By.id("_id_id"))).selectByVisibleText(service.trim());
		...
		if (service.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._id_idformError.parentFormmasterContractSubType.formError"));
			...
			Logger.debug("Service is Mandatory");
		...
		if (serviceOp.equalsIgnoreCase("")) {
			...
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("Service Organization Preference is Mandatory");
		...
		driver.findElement(By.className("dataTables_filter")).findElement(By.tagName("label")).findElement(By.tagName("input")).sendKeys(serviceOp);
		Thread.sleep(2000);
        ...
        Logger.debug("Service created successfully, with Name -- " + serviceOp);
