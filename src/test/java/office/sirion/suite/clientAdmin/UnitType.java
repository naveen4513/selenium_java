...
public class UnitType extends TestSuiteBase {
	...
	public void UnitTypeCreation (String unitType, String unitTypeOp, String unitTypeActive) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Client Admin Unit Type Creation Test -- "+unitTypeOp);
		...
		if (!unitType.equalsIgnoreCase("")) {
            try {
                new Select(driver.findElement(By.id("_id_id"))).selectByVisibleText(unitType.trim());
		...
		if (unitType.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._id_idformError.parentFormunitType.formError"));
			...
			Logger.debug("Unit Type is Mandatory");
		...
		if (unitTypeOp.equalsIgnoreCase("")) {
			...
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("Unit Type Organization Preference is Mandatory");
        ...
        Logger.debug("Unit Type created successfully, with Name -- " + unitTypeOp);
