...
public class InvoiceCommunication extends TestSuiteBase {
	String result = null;
	...
	public void InvoiceCommunicationTest (String Privacy, String Comments, String ActualDate, String RequestedBy, String ChangeRequest,
	    ...
	    Logger.debug("Executing Test Case Invoice Communication for -- " + ParentEntity);
		...
		// INVOICES - SHOW PAGE - COMMENTS AND ATTACHMENTS
	    ...
	    if (locateElementBy("entity_create_page_error_notifications")!=null) {
			...
			for (WebElement element : elementErrors) {
				...
				if (entityErrors.contains("Please add a comment or upload files"))
					Logger.debug("For Invoice -- " + ParentEntity + " -- Please add a comment or upload files");
				}
	    ...
	    Logger.debug("Test Case Invoice Communication with Name -- " + ParentEntity + " -- is STARTED");

	    verifyCommunicationTab(Comments, UploadFiles);

	    Logger.debug("Test Case Invoice Communication with Name -- " + ParentEntity + " -- is PASSED");
	    driver.get(CONFIG.getProperty("endUserURL"));
	    }
