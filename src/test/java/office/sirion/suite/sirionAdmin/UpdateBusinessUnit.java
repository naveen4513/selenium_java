...
public class UpdateBusinessUnit extends TestSuiteBase {
	...
	public void UpdateBusinessUnitTest (String businessUnit, String businessUnitUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +businessUnitUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Business Unit Update Test -- "+businessUnit);
...
//        String entityTypeShowPage = driver.findElement(By.id("s")).getText();
		String entityTypeShowPage = driver.findElement(By.id("_c_com_sirionlabs_business_entity_model_workflow_name_name_id")).getText();
        Assert.assertEquals(entityTypeShowPage, businessUnit, "Business Unit Name at show page is -- " +entityTypeShowPage+ " instead of -- " +businessUnit);
		...
		if (!businessUnitUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(businessUnitUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (businessUnitUpdate.length() > 128) {
			...
			Logger.debug("For Business Unit, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Business Unit, " +businessUnitUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, businessUnitUpdate, "Business Unit Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + businessUnitUpdate);

		Logger.debug("Business Unit updated successfully, with Name -- " + businessUnitUpdate);
