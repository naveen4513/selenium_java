...
public class ServiceCategory extends TestSuiteBase {
	...
	public void ServiceCategoryCreation (String serviceCategory) throws InterruptedException {
		count++;
		...
		if (!serviceCategory.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(serviceCategory.trim());
			}
		...
		if (serviceCategory.equalsIgnoreCase("")) {
			...
			Logger.debug("Service Category Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (serviceCategory.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Service Category, " +serviceCategory+" --- This name already exists in the system.");
        ...
        Logger.debug("Service Category created successfully, with Name -- " +serviceCategory);
