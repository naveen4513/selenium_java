...
public class SLSubCategory extends TestSuiteBase {
	...
	public void SLSubCategoryCreation (String slSubCategory
			) throws InterruptedException {

		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +slSubCategory +" set to NO " +count);
		...
		Logger.debug("Executing Test Sirion Admin SL Sub-Category Creation for -- " + slSubCategory);
		...
		if (!slSubCategory.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(slSubCategory.trim());
			}
		...
		if (slSubCategory.equalsIgnoreCase("")) {
			...
			Logger.debug("SL Sub-Category Name is Mandatory");

			driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }

		if (slSubCategory.length() > 256) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();

			Assert.assertEquals(entityErrors, "Maximum 256 characters allowed");
			Logger.debug("For SL Sub-Category, -- Maximum 256 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For SL Sub-Category, " +slSubCategory+" --- This name already exists in the system.");
		...
		Logger.debug("Test Sirion Admin SL Sub-Category Creation for -- " + slSubCategory + " is PASSED");
		driver.get(CONFIG.getProperty("sirionAdminURL"));
		}
