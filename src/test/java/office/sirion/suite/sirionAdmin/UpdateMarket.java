...
public class UpdateMarket extends TestSuiteBase {
	...
	public void UpdateMarketTest (String market, String marketUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +marketUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Market Update Test -- "+market);
		
		driver.get(CONFIG.getProperty("sirionAdminURL"));
		fluentWaitMethod(driver.findElement(By.linkText("Market")));
        ...
        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_market_name_name_id")).getText();
        Assert.assertEquals(entityTypeShowPage, market, "Market Name at show page is -- " +entityTypeShowPage+ " instead of -- " +market);
		...
		if (!marketUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(marketUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (marketUpdate.length() > 128) {
			...
			Logger.debug("For Market, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Market, " +marketUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_market_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, marketUpdate, "Market Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + marketUpdate);

		Logger.debug("Market updated successfully, with Name -- " + marketUpdate);
