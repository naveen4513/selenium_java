...
public class UpdateCRClass extends TestSuiteBase {
	...
	public void UpdateCRClassTest (String crClass, String crClassUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +crClassUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin CR Class Update Test -- "+crClass);
		...
		if (!crClassUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(crClassUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (crClassUpdate.length() > 128) {
			...
			Logger.debug("For CR Class, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For CR Class, " +crClassUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, crClassUpdate, "CR Class Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + crClassUpdate);

		Logger.debug("CR Class updated successfully, with Name -- " + crClassUpdate);
