...
public class UpdateInterpretationType extends TestSuiteBase {
	...
	public void UpdateInterpretationTypeTest (String interpretationType, String interpretationTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +interpretationTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Interpretation Type Update Test -- "+interpretationType);
        ...
        Assert.assertEquals(entityTypeShowPage, interpretationType, "Interpretation Type Name at show page is -- " +entityTypeShowPage+ " instead of -- " +interpretationType);
		...
		if (!interpretationTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(interpretationTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (interpretationTypeUpdate.length() > 128) {
			...
			Logger.debug("For Interpretation Type, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Interpretation Type, " +interpretationTypeUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, interpretationTypeUpdate, "Interpretation Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + interpretationTypeUpdate);

		Logger.debug("Interpretation Type updated successfully, with Name -- " + interpretationTypeUpdate);
