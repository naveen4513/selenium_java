...
public class UpdateCRType extends TestSuiteBase {
	...
	public void UpdateCRTypeTest (String crType, String crTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +crTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin CR Type Update Test -- "+crType);
		...
		if (!crTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(crTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (crTypeUpdate.length() > 128) {
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For CR Type, " +crTypeUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, crTypeUpdate, "CR Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + crTypeUpdate);

		Logger.debug("CR Type updated successfully, with Name -- " + crTypeUpdate);
