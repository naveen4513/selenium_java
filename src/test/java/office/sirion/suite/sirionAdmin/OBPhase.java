...
public class OBPhase extends TestSuiteBase {
	...
	public void OBPhaseCreation (String obPhase) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +obPhase +" set to NO " +count);
		...
		if (!obPhase.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(obPhase.trim());
			}
		...
		if (obPhase.equalsIgnoreCase("")) {
			...
			Logger.debug("OB Phase Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (obPhase.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For OB Phase, " +obPhase+" --- This name already exists in the system.");
		...
		Thread.sleep(2000);
        try{
        	if (!driver.findElement(By.xpath(".//*[@id='l_com_sirionlabs_model_MasterObligationPhase']/tbody/tr/td[1]/a/div[@sort='"+ obPhase +"']")).isDisplayed());
        	{
        		driver.findElement(By.xpath(".//*[@id='l_com_sirionlabs_model_MasterObligationPhase']/tbody/tr/td[1]/a/div[@sort='"+ obPhase +"']")).click();
        	}
        }catch (Exception ex){
        	Logger.debug("For OB Phase, " +obPhase+" --- Not created in Sirion Admin - It is a Bug");
        }
        ...
        Logger.debug("OB Phase created successfully, with Name -- " +obPhase);
