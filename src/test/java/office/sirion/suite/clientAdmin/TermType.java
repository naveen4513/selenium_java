...
public class TermType extends TestSuiteBase {
	...
	public void TermTypeCreation (String termType, String termTypeOp, String termTypeActive) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Client Admin Term Type Creation Test -- "+termTypeOp);
		...
		if (!termType.equalsIgnoreCase("")) {
			new Select(driver.findElement(By.id("_id_id"))).selectByVisibleText(termType.trim());
			}
		...
		if (termType.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._id_idformError.parentFormtermType.formError"));
			...
			Logger.debug("Term Type is Mandatory");
		...
		if (termTypeOp.equalsIgnoreCase("")) {
			...
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("Term Type Organization Preference is Mandatory");
        ...
        Assert.assertEquals(entityTypeShowPage, termType, "Term Type at show page is -- " +entityTypeShowPage+ " instead of -- " +termType);
        ...
        Logger.debug("Term Type created successfully, with Name -- " + termTypeOp);
