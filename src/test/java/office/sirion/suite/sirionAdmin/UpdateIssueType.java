...
public class UpdateIssueType extends TestSuiteBase {
	...
	public void UpdateIssueTypeTest (String issueType, String issueTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +issueTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Issue Type Update Test -- "+issueType);
        ...
        driver.findElement(By.xpath(".//*[@id='l_com_sirionlabs_model_MasterGroup']/tbody/tr/td[1]/div[@sort='"+ issueType +"']/a")).click();
		...
		if (!issueTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(issueTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();

			
		if (issueTypeUpdate.length() > 128) {
			...
			Logger.debug("For Issue Type, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Issue Type, " +issueTypeUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, issueTypeUpdate, "Issue Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + issueTypeUpdate);

		Logger.debug("Issue Type updated successfully, with Name -- " + issueTypeUpdate);
