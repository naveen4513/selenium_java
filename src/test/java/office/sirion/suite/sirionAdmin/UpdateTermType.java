...
public class UpdateTermType extends TestSuiteBase {
	...
	public void UpdateTermTypeTest (String termType, String termTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +termTypeUpdate +" set to NO " +count);
		...
		if (!termTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(termTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (termTypeUpdate.length() > 128) {
			...
			Logger.debug("For Term Type, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Term Type, " +termTypeUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_TermType_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, termTypeUpdate, "Term Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + termTypeUpdate);

		Logger.debug("Term Type updated successfully, with Name -- " + termTypeUpdate);
