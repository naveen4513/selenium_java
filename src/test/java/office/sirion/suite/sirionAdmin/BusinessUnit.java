...
public class BusinessUnit extends TestSuiteBase {
	...
	public void BusinessUnitCreation (String businessUnit) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Sirion Admin Business Unit Creation Test -- "+businessUnit);
		...
		if (!businessUnit.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(businessUnit.trim());
			}
		...
		if (businessUnit.equalsIgnoreCase("")) {
			...
			Logger.debug("Business Unit Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (businessUnit.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Business Unit, " +businessUnit+" --- This name already exists in the system.");
        ...
        Logger.debug("Business Unit created successfully, with Name -- " +businessUnit);
