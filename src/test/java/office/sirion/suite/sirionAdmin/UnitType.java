...
public class UnitType extends TestSuiteBase {
	...
	public void UnitTypeCreation (String unitType) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +unitType +" set to NO " +count);
		...
		Logger.debug("Executing Test Sirion Admin Unit Type Creation for -- "+unitType);
		...
		if (!unitType.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(unitType.trim());
			}
		...
		if (unitType.equalsIgnoreCase("")) {
			...
			Logger.debug("Unit Type Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (unitType.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
			
			Assert.assertEquals(entityErrors, "Maximum 128 characters allowed");
			Logger.debug("For Unit Type, -- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Unit Type, " +unitType+" -- This name already exists in the system.");
        ...
        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_UnitType_name_name_id")).getText();
        Assert.assertEquals(entityTypeShowPage, unitType, "Unit Type Name at show page is -- " +entityTypeShowPage+ " instead of -- " +unitType);

		Logger.debug("Test Sirion Admin Unit Type Creation for -- " + unitType + " is PASSED");
		driver.get(CONFIG.getProperty("sirionAdminURL"));
		}
