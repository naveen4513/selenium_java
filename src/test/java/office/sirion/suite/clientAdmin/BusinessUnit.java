...
public class BusinessUnit extends TestSuiteBase {
	...
	public void BusinessUnitCreation (String businessUnit, String businessUnitOp, String businessUnitActive) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Client Admin Business Unit Creation Test -- "+businessUnitOp);
		...
		if (!businessUnit.equalsIgnoreCase("")) {
			try {
                new Select(driver.findElement(By.id("_systemEntity.id_id"))).selectByVisibleText(businessUnit.trim());
		...
		if (businessUnit.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._systemEntity_id_idformError.parentFormbusinessUnit.formError"));
			...
			Logger.debug("Business Unit is Mandatory");
		...
		if (businessUnitOp.equalsIgnoreCase("")) {
			...
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("Business Unit Organization Preference is Mandatory");
        ...
        Logger.debug("Business Unit created successfully, with Name -- " + businessUnitOp);
