...
public class TransactionType extends TestSuiteBase {
	...
	public void TransactionTypeCreation (String transactionType) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +transactionType +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Transaction Type Creation Test -- "+transactionType);
		...
		if (!transactionType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(transactionType.trim());
			}
		...
		if (transactionType.equalsIgnoreCase("")) {
			...
			Logger.debug("Transaction Type Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (transactionType.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Transaction Type, " +transactionType+" --- This name already exists in the system.");
        ...
        Logger.debug("Transaction Type created successfully, with Name -- " +transactionType);
