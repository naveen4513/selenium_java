...
public class UpdateService extends TestSuiteBase {
	...
	public void UpdateServiceTest (String service, String serviceUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +serviceUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Service Creation Test -- "+service);
		
		driver.get(CONFIG.getProperty("sirionAdminURL"));
		fluentWaitMethod(driver.findElement(By.linkText("Service")));
        ...
        String entityTypeShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterContractSubType_name_name_id")).getText();
		...
		if (!serviceUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(serviceUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (serviceUpdate.length() > 128) {
			...
			Logger.debug("For Service, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Service, " +serviceUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_MasterContractSubType_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, serviceUpdate, "Service Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + serviceUpdate);

		Logger.debug("Service updated successfully, with Name -- " + serviceUpdate);
