...
public class Tier extends TestSuiteBase {
	...
	public void TierCreation (String tier, String tierOp) throws InterruptedException {
		...
		Logger.debug("Executing Client Admin Tier Creation Test -- "+tierOp);
		...
		if (!tier.equalsIgnoreCase("")) {
            try {
                new Select(driver.findElement(By.id("_id_id"))).selectByVisibleText(tier.trim());
		...
		if (tier.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._id_idformError.parentFormtier.formError"));
			...
			Logger.debug("Tier is Mandatory");
		...
		if (tierOp.equalsIgnoreCase("")) {
			...
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("Tier Organization Preference is Mandatory");
        ...
        String entityTypeOrgPreferenceShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_tblties_name_name_id")).getText();
        Assert.assertEquals(entityTypeOrgPreferenceShowPage, tierOp, "Tier Name at show page is -- " +entityTypeOrgPreferenceShowPage+ " instead of -- " +tierOp);

        Logger.debug("Tier created successfully, with Name -- " + tierOp);
