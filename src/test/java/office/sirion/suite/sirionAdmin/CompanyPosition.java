...
public class CompanyPosition extends TestSuiteBase {
	...
	public void CompanyPositionCreation (String companyPosition) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Sirion Admin Company Position Creation Test -- "+companyPosition);
		...
		if (!companyPosition.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(companyPosition.trim());
			}
		...
		if (companyPosition.equalsIgnoreCase("")) {
			...
			Logger.debug("Company Position Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (companyPosition.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Company Position, " +companyPosition+" --- This name already exists in the system.");
        ...
        Logger.debug("Company Position created successfully, with Name -- " +companyPosition);
