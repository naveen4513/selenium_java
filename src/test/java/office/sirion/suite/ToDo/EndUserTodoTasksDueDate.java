...
public class EndUserTodoTasksDueDate extends TestSuiteBase {
    ...
    public void EndUserTodoTasksDueDateTest() throws InterruptedException {
        ...
        if (!driver.findElement(By.xpath("//*[@id='thisweek']/div")).getText().equalsIgnoreCase("No events")) {
            ...
            if (!locateElementBy("end_user_next7days_week_task_count_home_page").getText().equalsIgnoreCase("")) {

                String weekTaskCount = locateElementBy("end_user_next7days_week_task_count_home_page").getText();
                int weekTaskcount = Integer.parseInt(weekTaskCount.replaceAll(",", ""));

                locateElementBy("end_user_next7days_tasks_link").click();


                String weektaskCount = locateElementBy("end_user_next7days_tasks_count").getText();
                String[] weektaskcount = weektaskCount.split(" ");
                String weekTaskSplitString = weektaskcount[4];

                int weektasksplitcount = Integer.parseInt(weekTaskSplitString);
                compareIntegerVals(weekTaskcount, weektasksplitcount);

                String Entity_ID = locateElementBy("end_user_weektasks_entity_id").getText();
                locateElementBy("end_user_weektasks_entity_id").click();
                ...
                try {
                    ...
                    if (driver.getPageSource().contains(Entity_ID)) {
                        ...
                        System.out.println("There is no data available under Next 7 Day's Approval");
                        fail = false;
                        driver.get(CONFIG.getProperty("endUserURL"));
