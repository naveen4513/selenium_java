...
public class ContractPaper extends TestSuiteBase {
	...
	public void ContractPaperCreation (String contractPaper
			) throws InterruptedException {

		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +contractPaper +" set to NO " +count);
		...
		Logger.debug("Executing Test Sirion Admin Contract Paper Creation for -- " + contractPaper);
		...
		if (!contractPaper.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(contractPaper.trim());
			}
		...
		if (contractPaper.equalsIgnoreCase("")) {
			...
			Logger.debug("Contract Paper Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (contractPaper.length() > 256) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Contract Paper, " +contractPaper+" -- This name already exists in the system.");
		...
		Logger.debug("Test Sirion Admin Contract Paper Creation for -- " + contractPaper + " is PASSED");
		driver.get(CONFIG.getProperty("sirionAdminURL"));
		}
