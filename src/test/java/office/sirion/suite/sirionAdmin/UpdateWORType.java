...
public class UpdateWORType extends TestSuiteBase {
	...
	public void UpdateWORTypeTest (String worType, String worTypeUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +worTypeUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin WOR Type Update Test -- "+worType);
		...
		if (!worTypeUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(worTypeUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (worTypeUpdate.length() > 128) {
			...
			Logger.debug("For WOR Type, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For WOR Type, " +worTypeUpdate+" --- This name already exists in the system.");
		...
		Assert.assertEquals(entityTypeUpdateShowPage, worTypeUpdate, "WOR Type Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + worTypeUpdate);

		Logger.debug("WOR Type updated successfully, with Name -- " + worTypeUpdate);
