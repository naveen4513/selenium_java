...
public class StatusType extends TestSuiteBase {
	...
	public void StatusTypeCreation (String statusType
			) throws InterruptedException {

		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +statusType +" set to NO " +count);

		openBrowser();
		clientAdminLogin(CONFIG.getProperty("clientAdminURL"), CONFIG.getProperty("clientAdminUsername"), CONFIG.getProperty("clientAdminPassword"));

		Logger.debug("Executing Test Client Admin Status Type Creation for -- "+statusType);

		driver.get(CONFIG.getProperty("clientAdminURL"));
		//
		fluentWaitMethod(driver.findElement(By.linkText("Status Type")));

		driver.findElement(By.linkText("Status Type")).click();
		//
		...
		if (!statusType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(statusType.trim());
			}
        ...
        if(driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			if (entityErrors.contains("This name already exists in the system."))
				Logger.debug("For Status Type, " +statusType+" -- This name already exists in the system.");
		...
		Assert.assertEquals(entityAlert, "Status Type created successfully");

		driver.findElement(By.cssSelector("button.ui-button.ui-widget.ui-state-default.ui-corner-all.ui-button-text-only")).click();

		Logger.debug("Test Client Admin Status Type Creation for -- " + statusType + " is PASSED");
        driver.get(CONFIG.getProperty("clientAdminURL"));
		}
