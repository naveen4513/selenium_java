...
public class TemplateCategory extends TestSuiteBase {
	...
	public void TemplateCategoryCreation (String templateCategory) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Sirion Admin Template Category Creation Test -- "+templateCategory);
		...
		if (!templateCategory.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(templateCategory.trim());
			}
		...
		if (templateCategory.equalsIgnoreCase("")) {
			...
			Logger.debug("Template Category Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (templateCategory.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Template Category, " +templateCategory+" --- This name already exists in the system.");
        ...
        Logger.debug("Template Category created successfully, with Name -- " +templateCategory);
