...
public class OBCategory extends TestSuiteBase {
	...
	public void OBCategoryCreation (String obCategory) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +obCategory +" set to NO " +count);
		...
		if (!obCategory.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(obCategory.trim());
			}
		...
		if (obCategory.equalsIgnoreCase("")) {
			...
			Logger.debug("OB Category Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (obCategory.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For OB Category, " +obCategory+" --- This name already exists in the system.");
        ...
        Logger.debug("OB Category created successfully, with Name -- " +obCategory);
