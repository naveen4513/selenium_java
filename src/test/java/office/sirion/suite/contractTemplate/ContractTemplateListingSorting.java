...
public class ContractTemplateListingSorting extends TestSuiteBase {
	String result = null;
	...
	public void ContractTemplateListingSortingTest (
	    ...
	    Logger.debug("Executing Test Case Listing Sorting of CONTRACT TEMPLATE");
	    ...
	    Logger.debug("Total Number of Entries Present under Listing -- CONTRACT TEMPLATE -- is = " + totalEntries);
	    ...
	    if (downloadDisplayAttribute[0].trim().equalsIgnoreCase("list-item")) {
	    	...
	    	if (file.exists()) {
	    		File selectAllColumnsFolder = new File(System.getProperty("user.dir")+"//Downloaded Data//"+new SimpleDateFormat("dd_MMMM_yyyy").format(Calendar.getInstance().getTime()).toString() + "//Common Listing//Selected Columns");
	    		...
	    		if (file.exists()) {
	    			File allColumnsFolder = new File(System.getProperty("user.dir")+"//Downloaded Data//"+new SimpleDateFormat("dd_MMMM_yyyy").format(Calendar.getInstance().getTime()).toString() + "//Common Listing//All Columns");
	    		...
	    		Logger.debug("Listing Page Downloaded Successfully for -- CONTRACT TEMPLATE");
	    		}
    	...
    	Logger.info("For CONTRACT TEMPLATE -- Total Columns = " + columnCount);
	    ...
	    for(int itr=1; itr <= totalIterations; itr++) {
	    	...
	    	for (int i = 0; i < columnSelection.size(); i++) {
	    		...
	    		if (!result) {
	    	    	Logger.error("Sorting of CONTRACT TEMPLATE is Failed at Column Name -- " + columnHeader[0]);
	    			resultCount++;
	    			}
	    ...
	    if (resultCount > 0)
	    	Assert.fail("Sorting of CONTRACT TEMPLATE is Failed");
	    else
	    	Logger.debug("Sorting of CONTRACT TEMPLATE is Completed");
