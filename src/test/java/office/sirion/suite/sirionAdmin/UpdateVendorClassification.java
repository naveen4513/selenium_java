...
public class UpdateVendorClassification extends TestSuiteBase {
	...
	public void UpdateVendorClassificationTest (String vendorClassification, String vendorClassificationUpdate) throws InterruptedException {
		count++;
		if(!runmodes[count].equalsIgnoreCase("Y"))
			throw new SkipException("Runmode for Test Data -- " +vendorClassificationUpdate +" set to NO " +count);
		...
		Logger.debug("Executing Sirion Admin Vendor Classification Creation Test -- "+vendorClassification);
		...
		if (!vendorClassificationUpdate.equalsIgnoreCase("")) {
			driver.findElement(By.id("_name_id")).clear();
			driver.findElement(By.id("_name_id")).sendKeys(vendorClassificationUpdate.trim());
			}

		driver.findElement(By.id("proceed")).click();
		//
			
		if (vendorClassificationUpdate.length() > 128) {
			...
			Logger.debug("For Vendor Classification, --- Maximum 128 characters allowed");
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Vendor Classification, " +vendorClassificationUpdate+" --- This name already exists in the system.");
						
			driver.get(CONFIG.getProperty("sirionAdminURL"));
			return;
			}

		String entityTypeUpdateShowPage = driver.findElement(By.id("_s_com_sirionlabs_model_VendorClassification_name_name_id")).getText();
		Assert.assertEquals(entityTypeUpdateShowPage, vendorClassificationUpdate, "Vendor Classification Name at show page is -- " + entityTypeUpdateShowPage + 
				" instead of -- " + vendorClassificationUpdate);

		Logger.debug("Vendor Classification updated successfully, with Name -- " + vendorClassificationUpdate);
