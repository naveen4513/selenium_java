...
public class IndustryType extends TestSuiteBase {
	...
	public void IndustryTypeCreation (String industryType) throws InterruptedException {
		count++;
		...
		Logger.debug("Executing Sirion Admin Industry Type Creation Test -- "+industryType);
		...
		if (!industryType.equalsIgnoreCase("")) {
			...
			driver.findElement(By.id("_name_id")).sendKeys(industryType.trim());
			}
		...
		if (industryType.equalsIgnoreCase("")) {
			...
			Logger.debug("Industry Type Name is Mandatory");
			
		    driver.get(CONFIG.getProperty("sirionAdminURL"));
		    return;
		    }
			
		if (industryType.length() > 128) {
			String entityErrors = driver.findElement(By.className("formErrorContent")).getText();
		...
		if(driver.findElements(By.id("errorDialog")).size()!=0 || driver.findElements(By.id("alertdialog")).size()!=0) {
			...
			Logger.debug("For Industry Type, " +industryType+" --- This name already exists in the system.");
        ...
        Logger.debug("Industry Type created successfully, with Name -- " +industryType);
