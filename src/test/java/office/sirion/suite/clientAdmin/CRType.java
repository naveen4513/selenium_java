...
public class CRType extends TestSuiteBase {
	...
	public void CRTypeCreation (String crType, String crTypeOp, String crTypeActive) throws InterruptedException {
		...
		Logger.debug("Executing Client Admin CR Type Creation Test -- "+crTypeOp);
		...
		if (!crType.equalsIgnoreCase("")) {
            try {
                new Select(driver.findElement(By.id("_systemEntity.id_id"))).selectByVisibleText(crType.trim());
		...
		if (crType.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._systemEntity_id_idformError.parentFormcrType.formError"));
			...
			Logger.debug("CR Type is Mandatory");
		...
		if (crTypeOp.equalsIgnoreCase("")) {
			...
			WebElement e = driver.findElement(By.cssSelector("div._name_idformError.parentFormcrType.formError"));
			String entityErrors = e.findElement(By.className("formErrorContent")).getText();
			
			Assert.assertEquals(entityErrors, "This field is required");
			Logger.debug("CR Type Organization Preference is Mandatory");
        ...
        Logger.debug("CR Type created successfully, with Name -- " + crTypeOp);
